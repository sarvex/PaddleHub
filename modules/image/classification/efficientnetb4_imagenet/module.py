...
inp_shape = {"b4": [380, 190, 190, 95, 48, 24, 24, 12]}
...
@moduleinfo(
    ...
    author="paddlepaddle",
    author_email="",
    summary="efficientnetb4_imagenet is a classification model, "
    "this module is trained with Imagenet dataset.",
    version="1.1.0",
    meta=ImageClassifierModule)
class EfficientNet_B4(nn.Layer):
    def __init__(self,
                 is_test: bool = False,
                 padding_type: str = "SAME",
        ...
        if load_checkpoint is not None:
            ...
            checkpoint = os.path.join(self.directory, 'efficientnet_b4_imagenet.pdparams')
            if not os.path.exists(checkpoint):
                os.system(
                    f'wget https://paddlehub.bj.bcebos.com/dygraph/image_classification/efficientnet_b4_imagenet.pdparams -O {checkpoint}'
                )
